...
vi.mock('@siemens/ix-react', () => ({
  ...
  IxApplication: (props: any) => <div {...props}>{props.children}</div>,
  // Header component: exposes `name` prop and child header controls
  IxApplicationHeader: (props: any) => <header>{props.name}{props.children}</header>,
  // Icon button: we forward onClick and render children so tests can click it
  IxIconButton: (props: any) => <button onClick={props.onClick} {...props}>{props.children}</button>,
}));
...
vi.mock('./components/FileImport/FileImport', () => {
  ...
  return {
    ...
    default: React.forwardRef((props: any, _ref: any) => (
      <div data-testid="file-import" data-open={props.isDialogOpen ? 'true' : 'false'} />
    )),
  };
});
